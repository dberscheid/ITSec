...
The seminar paper is based on the framework of Sebastian Gross\cite{carl3}. At the current state CORIAv3 does not include any automated ETL process for any source. The extraction part is handled manually by downloading the desired files and, if neccessary, unzipping the text-formatted file that for instance can be an edge list. \linebreak
...
Coriav3 already provides modules that can upload data into a MySQL or Redis database. Unexperienced users do not need to be familiar with database management systems.
